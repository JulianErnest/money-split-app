...
<objective>
Replace all loading spinners with shimmer skeleton loaders, add a custom pull-to-refresh animation, and wire haptic feedback into key user actions across the app.

Purpose: Skeleton loaders feel faster and more polished than spinners. Haptics provide tactile confirmation of actions. Pull-to-refresh with a custom animation adds personality.
Output: Skeleton components, custom PTR, haptic feedback on key actions. All list screens feel instant and responsive.
</objective>
...
2. Create `components/ui/PullToRefresh.tsx`:
   ...
   - Design: A peso coin (text "₱") that rotates/scales as user pulls down
  ...
  <done>
    - Skeleton.tsx exports GroupCardSkeleton, GroupsListSkeleton, GroupDetailSkeleton, ExpenseCardSkeleton
...
<task type="auto">
  <name>Task 2: Wire skeletons, pull-to-refresh, and haptics into screens</name>
  <files>
    app/(tabs)/index.tsx
    app/group/[id].tsx
  </files>
...
3. Haptic feedback patterns (per research discretion):
  ...
  <done>
    - No ActivityIndicator or spinner visible anywhere in the app
...
3. Pull-to-refresh on groups list and group detail
4. Haptics fire on key actions (requires physical device to verify)
5. Fade-in animation when data updates in background
6. `npx tsc --noEmit` passes
</verification>

<success_criteria>
- Zero loading spinners in the app (all replaced by skeletons)
