...
1. Update `app/(tabs)/index.tsx`:
   ...
   b) Replace the `ListEmptyComponent` with Taglish empty state:
      - Use `<EmptyState emoji={"🪙"} headline="Wala ka pang group!" subtext="Tap + to create one, or ask a friend for an invite code" />`

2. Update `app/group/[id].tsx`:
   a) Replace empty states with Taglish microcopy:
      - Balances "All settled up" -> `<EmptyState emoji={"✅"} headline="All settled!" subtext="Walang utang-utangan. Nice!" />`
      - Expenses "No expenses yet" -> `<EmptyState emoji={"🧾"} headline="No expenses yet" subtext="Mag-add ng expense para ma-track kung sino may utang" />`
      - Members empty state (just you): Not needed since creator is always a member

   b) Audit peso sign usage: ensure `formatPeso` from expense-utils is used consistently (it already returns formatted peso amounts). The balance display already uses "P" prefix via formatPeso -- update any hardcoded "P" to use the peso sign "₱" if not already handled by formatPeso.

3. Update `app/group/[id]/balance/[memberId].tsx`:
   - Audit all currency displays for peso sign consistency
   - Replace any hardcoded 'P' prefix with the proper peso sign '₱'
   - Check if `formatPeso` utility is used; if the file formats amounts manually (e.g., `'P' + amount` or template literals with 'P'), replace with '₱' or call `formatPeso`
   - If the file uses a local formatting function, update it to use '₱' instead of 'P'
   - This addresses the UX-08 gap: balance drill-down must show '₱' not 'P'
...
5. Update `app/group/[id].tsx` to use the renamed `AddMemberSheet` with ref pattern:
  ...
  <verify>
    ...
    - Balance drill-down screen shows '₱' not 'P' for all amounts
  </verify>
  <done>
    ...
    - Peso sign '₱' used consistently throughout, including balance drill-down
...
6. Balance drill-down screen shows '₱' (not 'P') for all amounts
...
- Peso sign '₱' visible in ALL currency contexts including balance drill-down
</success_criteria>
