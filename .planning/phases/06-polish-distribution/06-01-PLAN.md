...
must_haves:
  truths:
    - "App shows a persistent offline banner when device loses connectivity"
    - "Offline queue can store add-expense and create-group actions in SQLite"
    - "Cached data layer reads/writes groups, expenses, balances to SQLite keyed by user ID"
